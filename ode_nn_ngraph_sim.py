...
class ODEfunc(nn.Module):
    ...
    def forward(self, t, x):
        ...
        dS = -beta.unsqueeze(-1)*(torch.multiply(AI,S)) #/np.shape(self.A)[0]
        ...
        #dI = self.ln(dI)
        #dR = self.ln(dR)
        return torch.cat((dS,dI,dR,torch.zeros_like(x[int(x.size()[0]/4)*3:,:])))
...
class ODEBlock(nn.Module):
    ...
    def init_weights(self):
        ...
        nn.init.kaiming_normal_(self.linearS1.weight, mode='fan_in', 
                                 nonlinearity='relu')
        nn.init.kaiming_normal_(self.linear3.weight, mode='fan_in', 
                                 nonlinearity='relu')
        self.linearS2.weight.data.normal_(0, 1)

    def forward(self, x):
        x = x.view(-1, x.size(2))
        S0, I0, R0, beta_gamma_batch = x[:,0], x[:,1], x[:,2], x[:,3:] #torch.zeros(x.size()[0], self.hidden1, device=self.device)
        ...
        sol = odeint(self.odefunc, torch.cat((S,I,R,beta_gamma_batch)), self.integration_time, method='euler')

        S, I, R = sol[:,:int(sol.size()[1]/4),:], sol[:,int(sol.size()[1]/4):2*int(sol.size()[1]/4),:], sol[:,2*int(sol.size()[1]/4):3*int(sol.size()[1]/4),:]

        S = self.relu3(self.linear3(S))
        I = self.relu3(self.linear3(I))
        R = self.relu3(self.linear3(R)) 
...
def load_SIR_labels(dataset, path_to_save, G, I_indices, beta, gamma, sim, maxTime):
    if os.path.exists(path_to_save + '/' + dataset[14:] + '-S-' + '-'.join(str(i) for i in I_indices) + '.pkl'):
        S_labels = pickle.load(open(path_to_save + '/' + dataset[14:] + '-S-' + '-'.join(str(i) for i in I_indices) + ".pkl", "rb")) #/sim
        I_labels = pickle.load(open(path_to_save + '/' + dataset[14:] + '-I-' + '-'.join(str(i) for i in I_indices) + ".pkl", "rb")) #/sim
        R_labels = pickle.load(open(path_to_save + '/' + dataset[14:] + '-R-' + '-'.join(str(i) for i in I_indices) + ".pkl", "rb")) #/sim
        print('ok')
    else:
...
def train(model, optimizer, criterion, device, train_loader, val_loader, maxTime, deltaT, n_nodes):
    ...
    for x, y in train_loader:
        ...
        #print(S_pred_t)
        loss = criterion(torch.transpose(torch.cat((torch.unsqueeze(S_pred_t,-1), torch.unsqueeze(I_pred_t,-1), torch.unsqueeze(R_pred_t,-1)), -1), 0, 1)[:,1:,:].to(device), y.view(-1, y.size(2), y.size(3))[:,1:,:])
        
        if accumulation_steps!=0:
            loss.backward()
            if (samples + 1) % accumulation_steps == 0:
                optimizer.step()
    ...
    for x, y in val_loader:
        ...
        val_loss = criterion(torch.transpose(torch.cat((torch.unsqueeze(S_pred_t,-1), torch.unsqueeze(I_pred_t,-1), torch.unsqueeze(R_pred_t,-1)), -1), 0, 1)[:,1:,:].to(device), y.view(-1, y.size(2), y.size(3))[:,1:,:])
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best_loss:
            ...
            test_loss, test_loss_all = test(model, criterion, device, test_loader, args.maxTime, args.deltaT, n_nodes)
